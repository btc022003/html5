...
```
mongoose支持promise，
可以使用 mongoose.Promise = Promise;实现替换其原生的内置promise插件.
如果不替换会有警告提示:
  (node:2215) DeprecationWarning: Mongoose: mpromise (mongoose's default promise library) is deprecated, plug in your own promise library instead: http://mongoosejs.com/docs/promises.html
mongoose中模型的定义和rails中的很像,你只需要定义好模型的名字,其会自动创建数据库集合,生成模型名字复数形式的集合名.此使用方法倍感亲切！
